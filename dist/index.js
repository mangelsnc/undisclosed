...
function keysExists() {
    return fs_1.default.existsSync(privateKeyPath) || fs_1.default.existsSync(publicKeyPath);
}
function encryptedFileExists() {
    return fs_1.default.existsSync(encryptedDataPath);
}
...
function loadSecrets(truncateValue = false) {
    const readStream = fs_1.default.readFileSync(encryptedDataPath);
...
function handleSet() {
    ...
    const publicKey = fs_1.default.readFileSync(publicKeyPath, 'utf8').toString();
    const key = args[3];
    const value = args[4];
    const encryptedValue = encrypt(value, publicKey);
    fs_1.default.appendFileSync(encryptedDataPath, key.toUpperCase() + '=' + encryptedValue + '\n');
...
function handleGet() {
    ...
    const privateKey = fs_1.default.readFileSync(privateKeyPath, 'utf8').toString();
...
function handleDump() {
    ...
    fs_1.default.writeFileSync(decryptedDataPath, dumpedContent.join('\n'));
